...
namespace Seb.Fluid.Rendering
{
    ...
    public class BucketRenderer : MonoBehaviour
    {
        ...
        [Tooltip("Transparent glass bucket so the fluid inside is visible (skips the occlusion mask).")]
        public bool glass = false;
        [Range(0f, 1f)] public float glassAlpha = 0.18f;
        ...
        void LateUpdate()
        {
            ...
            // Pass 1 — depth mask (opaque bucket only): writes depth before particles so the fluid
            // behind the walls is hidden. A glass bucket SKIPS this so the fluid inside stays visible.
            if (!glass)
            {
            ...
            // Pass 2 — visual (transparent glass after particles, or opaque after the mask)
        ...
        Material MakeVisMat(Color col)
        {
            ...
            if (glass)
            {
                // Transparent glossy glass — see the fluid through it.
                ...
                mat.color       = new Color(col.r, col.g, col.b, glassAlpha);
