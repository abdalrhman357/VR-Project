...
namespace Seb.Fluid.Simulation
{
	public class FluidSim : MonoBehaviour
	{
		...
		[Tooltip("Container shape: false = cylinder (bucket), true = axis-aligned box (fluid sandbox).")]
		public bool useBoxBounds = false;
		[Tooltip("Recycle particles that fall below recycleHeight back into the bucket top (continuous paint source).")]
		public bool recycleParticles = false;
		public float recycleHeight = -1f;
		...
		/// <summary>
		/// Refill: re-read the spawner and re-upload particles. If the particle count is unchanged
		/// (same bucket size), the existing GPU buffers are reused so dependent renderers stay valid.
		/// If the count changed (e.g. bucket resized), the simulation is fully rebuilt.
		/// Used by SimulationManager.ResetSimulation so "Reset" actually refills the bucket.
		/// </summary>
		public void ResetParticles()
		{
			if (spawner == null) return;
			Spawner3D.SpawnData data = spawner.GetSpawnData();
			if (positionBuffer == null || data.points.Length != positionBuffer.count)
			{
				Reinitialize();
				return;
			}
		...
		/// <summary>Full GPU rebuild from the current spawner (handles a changed particle count).</summary>
		public void Reinitialize()
		{
			...
			Initialize();   // re-creates buffers and re-fires SimulationInitCompleted (rebinds listeners)
		}
		...
		void UpdateSettings(float stepDeltaTime, float frameDeltaTime)
		{
			...
			// Bottom hole + container shape
			compute.SetFloat("holeRadius",  holeRadius);
			compute.SetVector("holeOffset", new Vector4(holeOffset.x, holeOffset.y, 0, 0));
			compute.SetInt("useBoxBounds",  useBoxBounds ? 1 : 0);
			compute.SetInt("recycleActive", recycleParticles ? 1 : 0);
			compute.SetFloat("recycleHeight", recycleHeight);
		...
		void OnDrawGizmosSelected()   // only when the object is selected → no green wireframe cluttering Play view
		{
			var m = Gizmos.matrix;
			...
			if (useBoxBounds)
			{
				Gizmos.DrawWireCube(Vector3.zero, Vector3.one); // box container
				Gizmos.matrix = m;
				return;
			}
